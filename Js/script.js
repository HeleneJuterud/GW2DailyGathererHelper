  ...
  $(document).ready(function() {
   ...
   $('select').on('contentChanged', function() {
	...
	{
	var self = this;
    var clipboard = new Clipboard('.WaypointInfo');

    clipboard.on('success', function(e) {
      //To avoid several messages showing up if button spammed
...
// Make sure there's max 2 results on each line. By adding clear: both if true.
  ...
  self.selectedMinidungeon = ko.observable();
  ...
  self.MiniDungeonsOptions = ko.observableArray([
	...
	self.DailyOptions = ko.observableArray([
		{ id: "Miner", name: "Miner", iconName: "miningIcon"},
		{ id: "Forager", name: "Forager", iconName: "harvestIcon"},
		{ id: "Lumberer", name: "Lumberer", iconName: "loggingIcon"},
		{ id: "VistaViewer", name: "Vista Viewer", iconName: "VistaIcon"},
    { id: "Minidungeon", name: "Minidungeon", iconName: "dungeonicon"},
		]);

	self.AreaOptions = ko.observableArray([
		{ id: "Ascalon", name: "Ascalon" },
		{ id: "Kryta", name: "Kryta" },
		{ id: "Shiverpeaks", name: "Shiverpeaks"},
		{ id: "Orr", name: "Orr" },
		{ id: "Maguuma_Jungle", name: "Maguuma Jungle" },
		{ id: "Maguuma_Wastes", name: "Maguuma Wastes" },
		{ id: "Heart_of_Maguuma", name: "Heart of Maguuma" },
...
// Triggered when user click on daily. Updates search based to match selected daily.
  self.setSearchByDaily = function(daily){
      self.selectedDailyOption(self.DailyOptions().find(function(x){ return daily.name.includes(x.name)}))
        var area = null;
        self.selectedMinidungeon(null);
        if(daily.name.includes("Minidungeon"))
        {
            self.selectedMinidungeon(self.MiniDungeonsOptions().find(function(x){ return daily.name.includes(x.name)}));
            area = self.AreaOptions().find(function(x){ return x.name == self.selectedMinidungeon().area });
        }
        else
          area = self.AreaOptions().find(function(x){ return  daily.requirement.includes(x.name)});

        if(!area)
          area = self.AreaOptions().find(function(x){ return  daily.name.includes(x.name)});

        self.selectedAreaOption(area);
...
// Add icons to daily option dropdown.
	self.setDailyOptionicon = function (option, item) {
        ...
        ko.applyBindingsToNode(option, {
            attr: {
                'data-icon': 'images/icons/'+ item.iconName + '.png'
            }
        }, item);
    }

    self.setGatheringItemIcon = function(itemType){
        switch(itemType){
          case "Miner":
            return "images/icons/miningIcon.png";
            break;

          case "Forager":
            return "images/icons/harvestIcon.png";
            break;

          case "Lumberer":
            return "images/icons/loggingIcon.png";
            break;
          case "Vista Viewer":
            return "images/icons/VistaIcon.png";
            break;

          case "Guild hall":
          return "images/icons/guildhallIcon.png";
          break;

          case "Minidungeon":
          return "images/icons/achievement_chest.png";
          break;
...
// Filters search, based on users choice
 self.FilterResults =  ko.computed(() => {
    ...
    if(!self.selectedDailyOption() && !self.selectedAreaOption())
    ...
    else if(self.selectedDailyOption() && self.selectedAreaOption()){

        if(self.selectedMinidungeon() && self.selectedDailyOption().name == "Minidungeon" && self.selectedAreaOption().name == self.selectedMinidungeon().area){
            self.findDailyIndex(self.DailyGathererAchievements().findIndex(x => ( x.name.includes(self.selectedMinidungeon().name))));
           self.filteredResult(self.allLocations.Locations().filter(loc => loc.MapName() == self.selectedMinidungeon().name));
         }

        else{
            self.selectedMinidungeon(null);
             self.findDailyIndex(self.DailyGathererAchievements().findIndex(x => ((x.name.includes(self.selectedAreaOption().name) && x.name.includes(self.selectedDailyOption().name)))));
            self.filteredResult(self.allLocations.Locations().filter(loc => (loc.Type() == self.selectedDailyOption().name || (loc.Type() == "Guild hall" && self.selectedDailyOption().name != "Vista Viewer" && self.selectedDailyOption().name != "Minidungeon")) && loc.Area() == self.selectedAreaOption().name));
        }
        ...
        return self.filteredResult();
          ...
          self.filteredResult(self.allLocations.Locations().filter(loc => loc.Type() == self.selectedDailyOption().name || (loc.Type() == "Guild hall" && self.selectedDailyOption().name != "Vista Viewer" && self.selectedDailyOption().name != "Minidungeon")));
           return self.filteredResult();
      }
      else if(!self.selectedDailyOption() && self.selectedAreaOption()){
          self.filteredResult(self.allLocations.Locations().filter(loc => loc.Area() == self.selectedAreaOption().name));
           return self.filteredResult();
        }
    ...
    self.ResultImageSrc = function(imageName) {
        ...
        return "images/LocationImages/" + imageName;
