$(document).ready(function(){$("select").material_select(),$(".button-collapse").sideNav(),$("select").on("contentChanged",function(){// re-initialize (update)
$(this).material_select()})});var vm=function(){var a=this,b=new Clipboard(".WaypointInfo");b.on("success",function(a){//To avoid several messages showing up if button spammed
Materialize.Toast.removeAll(),Materialize.toast("Copied!",4e3),a.clearSelection()}),a.canBeDividedbytwo=function(a){return!(0!=a%2)},a.showAdvancedSearch=ko.observable(!1),a.selectedDailyOption=ko.observable(),a.selectedAreaOption=ko.observable(),a.selectedMinidungeon=ko.observable(),a.filteredResult=ko.observableArray(),a.DailyGathererAchievements=ko.observableArray(),a.findDailyIndex=ko.observable(-1),a.showingTodaysDaily=ko.observable(!0),a.MiniDungeonsOptions=ko.observableArray([{name:"Bad Neighborhood",area:"Kryta"},{name:"Goff's Loot",area:"Kryta"},{name:"Forsaken Fortune",area:"Shiverpeaks"},{name:"Windy Cave Treasure",area:"Shiverpeaks"},{name:"Vexa's Lab",area:"Ascalon"},{name:"Ship of Sorrows",area:"Orr"},{name:"Forgotten Stream",area:"Orr"},{name:"Don't Touch the Shiny",area:"Maguuma Jungle"},{name:"Grounded",area:"Orr"},{name:"Magellan's Memento",area:"Shiverpeaks"},{name:"Tears of Itlaocol",area:"Maguuma Jungle"},{name:"Rebel's Seclusion",area:"Ascalon"},{name:"The Long Way Around",area:"Orr"}]),a.DailyOptions=ko.observableArray([{id:"Miner",name:"Miner",iconName:"miningIcon"},{id:"Forager",name:"Forager",iconName:"harvestIcon"},{id:"Lumberer",name:"Lumberer",iconName:"loggingIcon"},{id:"VistaViewer",name:"Vista Viewer",iconName:"VistaIcon"},{id:"Minidungeon",name:"Minidungeon",iconName:"dungeonicon"}]),a.AreaOptions=ko.observableArray([{id:"Ascalon",name:"Ascalon"},{id:"Kryta",name:"Kryta"},{id:"Shiverpeaks",name:"Shiverpeaks"},{id:"Orr",name:"Orr"},{id:"Maguuma_Jungle",name:"Maguuma Jungle"},{id:"Maguuma_Wastes",name:"Maguuma Wastes"},{id:"Heart_of_Maguuma",name:"Heart of Maguuma"},{id:"Desert",name:"Desert"}]),a.toggleAdvancedSearch=function(){return a.showAdvancedSearch()?a.showAdvancedSearch(!1):a.showAdvancedSearch(!0)},a.setSearchByDaily=function(b){a.selectedDailyOption(a.DailyOptions().find(function(a){return b.name.includes(a.name)}));var c=null;// Scroll to result
// Triggers the Materialize update function
a.selectedMinidungeon(null),b.name.includes("Minidungeon")?(a.selectedMinidungeon(a.MiniDungeonsOptions().find(function(a){return b.name.includes(a.name)})),c=a.AreaOptions().find(function(b){return b.name==a.selectedMinidungeon().area})):c=a.AreaOptions().find(function(a){return b.requirement.includes(a.name)}),c||(c=a.AreaOptions().find(function(a){return b.name.includes(a.name)})),a.selectedAreaOption(c),$("html, body").animate({scrollTop:$(".resultContainer").offset().top},1e3),$("#AreaSelect").trigger("contentChanged"),$("#DailySelect").trigger("contentChanged")},a.findAchievementInfo=function(b){var c=!1;a.DailyGathererAchievements([]),$.get("https://api.guildwars2.com/v2/achievements?ids="+b,function(b){b.forEach(function(b){// Check if the element from API, contain any of the daily options.
c=a.DailyOptions().some(a=>b.name.includes(a.name)),c&&a.DailyGathererAchievements.push(b)})})},a.findDailyAchievements=function(b){switch(a.DailyGathererAchievements([]),b){case"today":var c="https://api.guildwars2.com/v2/achievements/daily";a.showingTodaysDaily(!0);break;case"tomorrow":var c="https://api.guildwars2.com/v2/achievements/daily/tomorrow";a.showingTodaysDaily(!1);break;default:var c="https://api.guildwars2.com/v2/achievements/daily";a.showingTodaysDaily(!0);}var d=[];$.get(c,function(b){b.pve.forEach(function(a){d.push(a.id)}),a.findAchievementInfo(d)})},a.setDailyOptionicon=function(a,b){b&&ko.applyBindingsToNode(a,{attr:{"data-icon":"images/icons/"+b.iconName+".png"}},b)},a.setGatheringItemIcon=function(a){switch(a){case"Miner":return"images/icons/miningIcon.png";break;case"Forager":return"images/icons/harvestIcon.png";break;case"Lumberer":return"images/icons/loggingIcon.png";break;case"Vista Viewer":return"images/icons/VistaIcon.png";break;case"Guild hall":return"images/icons/guildhallIcon.png";break;case"Minidungeon":return"images/icons/achievement_chest.png";break;default:return"";}},a.locationData=`
	{
	"Locations": [
...
}`,a.allLocations=ko.mapping.fromJSON(a.locationData),a.FilterResults=ko.computed(()=>{if(a.selectedDailyOption()||a.selectedAreaOption())return a.selectedDailyOption()&&a.selectedAreaOption()?(a.selectedMinidungeon()&&"Minidungeon"==a.selectedDailyOption().name&&a.selectedAreaOption().name==a.selectedMinidungeon().area?(a.findDailyIndex(a.DailyGathererAchievements().findIndex(b=>b.name.includes(a.selectedMinidungeon().name))),a.filteredResult(a.allLocations.Locations().filter(b=>b.MapName()==a.selectedMinidungeon().name))):(a.selectedMinidungeon(null),a.findDailyIndex(a.DailyGathererAchievements().findIndex(b=>b.name.includes(a.selectedAreaOption().name)&&b.name.includes(a.selectedDailyOption().name))),a.filteredResult(a.allLocations.Locations().filter(b=>(b.Type()==a.selectedDailyOption().name||"Guild hall"==b.Type()&&"Vista Viewer"!=a.selectedDailyOption().name&&"Minidungeon"!=a.selectedDailyOption().name)&&b.Area()==a.selectedAreaOption().name))),a.filteredResult()):a.selectedDailyOption()&&!a.selectedAreaOption()?(a.filteredResult(a.allLocations.Locations().filter(b=>b.Type()==a.selectedDailyOption().name||"Guild hall"==b.Type()&&"Vista Viewer"!=a.selectedDailyOption().name&&"Minidungeon"!=a.selectedDailyOption().name)),a.filteredResult()):!a.selectedDailyOption()&&a.selectedAreaOption()?(a.filteredResult(a.allLocations.Locations().filter(b=>b.Area()==a.selectedAreaOption().name)),a.filteredResult()):void 0},a).extend({rateLimit:300});// Set location image. If image does not exist set default
a.ResultImageSrc=function(a){return a?"images/LocationImages/"+a:"images/LocationImages/Applenook_NoImage.jpg"}},vm=new vm;ko.applyBindings(vm);
