$(document).ready(function(){$('select').material_select();$(".button-collapse").sideNav();$('select').on('contentChanged',function(){$(this).material_select()})});var vm=function()
{var self=this;var clipboard=new Clipboard('.WaypointInfo');clipboard.on('success',function(e){Materialize.Toast.removeAll();Materialize.toast('Copied!',4000)
e.clearSelection()});self.canBeDividedbytwo=function(number){return(number%2===0?!0:!1)}
self.showAdvancedSearch=ko.observable(!1);self.selectedDailyOption=ko.observable();self.selectedAreaOption=ko.observable();self.selectedMinidungeon=ko.observable();self.filteredResult=ko.observableArray();self.DailyGathererAchievements=ko.observableArray();self.findDailyIndex=ko.observable(-1);self.showingTodaysDaily=ko.observable(!0);self.MiniDungeonsOptions=ko.observableArray([{name:"Bad Neighborhood",area:"Kryta"},{name:"Goff's Loot",area:"Kryta"},{name:"Forsaken Fortune",area:"Shiverpeaks"},{name:"Windy Cave Treasure",area:"Shiverpeaks"},{name:"Vexa's Lab",area:"Ascalon"},{name:"Ship of Sorrows",area:"Orr"},{name:"Forgotten Stream",area:"Orr"},{name:"Don't Touch the Shiny",area:"Maguuma Jungle"},{name:"Grounded",area:"Orr"},{name:"Magellan's Memento",area:"Shiverpeaks"},{name:"Tears of Itlaocol",area:"Maguuma Jungle"},{name:"Rebel's Seclusion",area:"Ascalon"},{name:"The Long Way Around",area:"Orr"},]);self.DailyOptions=ko.observableArray([{id:"Miner",name:"Miner",iconName:"miningIcon"},{id:"Forager",name:"Forager",iconName:"harvestIcon"},{id:"Lumberer",name:"Lumberer",iconName:"loggingIcon"},{id:"VistaViewer",name:"Vista Viewer",iconName:"VistaIcon"},{id:"Minidungeon",name:"Minidungeon",iconName:"dungeonicon"},]);self.AreaOptions=ko.observableArray([{id:"Ascalon",name:"Ascalon"},{id:"Kryta",name:"Kryta"},{id:"Shiverpeaks",name:"Shiverpeaks"},{id:"Orr",name:"Orr"},{id:"Maguuma_Jungle",name:"Maguuma Jungle"},{id:"Maguuma_Wastes",name:"Maguuma Wastes"},{id:"Heart_of_Maguuma",name:"Heart of Maguuma"},{id:"Desert",name:"Desert"},]);self.toggleAdvancedSearch=function(){return(self.showAdvancedSearch()?self.showAdvancedSearch(!1):self.showAdvancedSearch(!0))};self.setSearchByDaily=function(daily){self.selectedDailyOption(self.DailyOptions().find(function(x){return daily.name.includes(x.name)}))
var area=null;self.selectedMinidungeon(null);if(daily.name.includes("Minidungeon"))
{self.selectedMinidungeon(self.MiniDungeonsOptions().find(function(x){return daily.name.includes(x.name)}));area=self.AreaOptions().find(function(x){return x.name==self.selectedMinidungeon().area})}
else area=self.AreaOptions().find(function(x){return daily.requirement.includes(x.name)});if(!area)
area=self.AreaOptions().find(function(x){return daily.name.includes(x.name)});self.selectedAreaOption(area);$('html, body').animate({scrollTop:$(".resultContainer").offset().top},1000);$("#AreaSelect").trigger('contentChanged');$("#DailySelect").trigger('contentChanged')}
self.findAchievementInfo=function(DailyIds){var isGathererDaily=!1;var hasLocation=!1;self.DailyGathererAchievements([]);$.get("https://api.guildwars2.com/v2/achievements?ids="+DailyIds,function(data){data.forEach(function(element){isGathererDaily=self.DailyOptions().some(x=>element.name.includes(x.name));if(isGathererDaily)
self.DailyGathererAchievements.push(element)})})};self.findDailyAchievements=function(when){self.DailyGathererAchievements([]);switch(when){case "today":var url="https://api.guildwars2.com/v2/achievements/daily";self.showingTodaysDaily(!0);break;case "tomorrow":var url="https://api.guildwars2.com/v2/achievements/daily/tomorrow";self.showingTodaysDaily(!1);break;default:var url="https://api.guildwars2.com/v2/achievements/daily";self.showingTodaysDaily(!0);break}
var dailyIds=[];$.get(url,function(data){data.pve.forEach(function(element){dailyIds.push(element.id)});self.findAchievementInfo(dailyIds)})}
self.setDailyOptionicon=function(option,item){if(!item)
return
ko.applyBindingsToNode(option,{attr:{'data-icon':'images/icons/'+item.iconName+'.png'}},item)}
self.setGatheringItemIcon=function(itemType){switch(itemType){case "Miner":return"images/icons/miningIcon.png";break;case "Forager":return"images/icons/harvestIcon.png";break;case "Lumberer":return"images/icons/loggingIcon.png";break;case "Vista Viewer":return"images/icons/VistaIcon.png";break;case "Guild hall":return"images/icons/guildhallIcon.png";break;case "Minidungeon":return"images/icons/achievement_chest.png";break;default:return"";break}};self.locationData=`
	{
	"Locations": [
...
self.FilterResults=ko.computed(()=>{if(!self.selectedDailyOption()&&!self.selectedAreaOption())
return;else if(self.selectedDailyOption()&&self.selectedAreaOption()){if(self.selectedMinidungeon()&&self.selectedDailyOption().name=="Minidungeon"&&self.selectedAreaOption().name==self.selectedMinidungeon().area){self.findDailyIndex(self.DailyGathererAchievements().findIndex(x=>(x.name.includes(self.selectedMinidungeon().name))));self.filteredResult(self.allLocations.Locations().filter(loc=>loc.MapName()==self.selectedMinidungeon().name))}
else{self.selectedMinidungeon(null);self.findDailyIndex(self.DailyGathererAchievements().findIndex(x=>((x.name.includes(self.selectedAreaOption().name)&&x.name.includes(self.selectedDailyOption().name)))));self.filteredResult(self.allLocations.Locations().filter(loc=>(loc.Type()==self.selectedDailyOption().name||(loc.Type()=="Guild hall"&&self.selectedDailyOption().name!="Vista Viewer"&&self.selectedDailyOption().name!="Minidungeon"))&&loc.Area()==self.selectedAreaOption().name))}
return self.filteredResult()}
else if(self.selectedDailyOption()&&!self.selectedAreaOption()){self.filteredResult(self.allLocations.Locations().filter(loc=>loc.Type()==self.selectedDailyOption().name||(loc.Type()=="Guild hall"&&self.selectedDailyOption().name!="Vista Viewer"&&self.selectedDailyOption().name!="Minidungeon")));return self.filteredResult()}
else if(!self.selectedDailyOption()&&self.selectedAreaOption()){self.filteredResult(self.allLocations.Locations().filter(loc=>loc.Area()==self.selectedAreaOption().name));return self.filteredResult()}},self).extend({rateLimit:300});self.ResultImageSrc=function(imageName){if(!imageName)
return"images/LocationImages/Applenook_NoImage.jpg";return"images/LocationImages/"+imageName}};var vm=new vm();ko.applyBindings(vm)
